...
### The Basic Formula:
If the input image is $n \times n$ and the filter is $f \times f$, the size of the output feature map is:
...
When we factor in both Padding ($p$) and Strides ($s$), the formula to calculate the exact dimension of the output Feature Map is:

$$
\text{Output Size} = \left( \frac{n + 2p - f}{s} \right) + 1
$$

*(Note: The result is usually rounded down (floor) if it does not divide perfectly).*
...
$$
\text{Output} = \left( \frac{7 + 2(1) - 3}{2} \right) + 1
$$

$$
\text{Output} = \left( \frac{9 - 3}{2} \right) + 1 = \left( \frac{6}{2} \right) + 1 = 4
$$
